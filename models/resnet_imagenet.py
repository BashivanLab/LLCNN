...
def _sine_init(w):
    ...
    k1, k2 = get_closest_factors(num_kernels)
    x = torch.arange(0, k2/(k2-1)*2*np.pi, 2*np.pi/(k2-1))
    b = np.zeros([k1,k2])
    for r in range(k1):
        b[r,:] = 0.01*torch.sin(x+r*x[1])
    w.data.add_(torch.Tensor(b.reshape(k1*k2,1,1,1)))
    return w

class PooledConv(nn.Module):
  def __init__ (self, in_planes, planes, kernel_size, stride, padding=0, bias=False, 
  groups=1, dilation=1, pool_type='mean', max_num_pools=1, noise_std=0.2, kap_kernelsize=3):
  ...
  def forward(self,x): 
    ...
    if self.pool_type is not None:
      ...
      if self.kap_kernelsize <= 1.: 
        k = round(min(self.kw, self.kh) * self.kap_kernelsize)
        if k % 2 == 0: 
            k = k-1
        k = max(k, 3)  # restrict minimum kap_kernelsize to 3
      else:
        k = self.kap_kernelsize

      c = 1
      pooled_output = 0
      while (c <= self.max_num_pools) & (k <= max_kernel_size):
        if self.pool_type == 'mean':
          pooled_output = pooled_output + F.avg_pool2d(reshaped_out, kernel_size=k, stride=1, padding=(k-1)//2, count_include_pad=False)  # 2 * k**2 * 
...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, pool_type='mean', max_num_pools=1, noise_std=0.2, kap_kernelsize=3):
...
pool_type='mean', max_num_pools=2, noise_std=0.2, kap_kernelsize=3):
    """1x1 convolution"""
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, planes, pool_type, max_num_pools, noise_std, kap_kernelsize=3, num_classes=1000, 
                 ...
                 norm_layer=None, sine_init=False):
        super(ResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        self.sine_init = sine_init
    ...
    def _make_layer(self, block, planes, blocks, pool_type, max_num_pools, noise_std, kap_kernelsize=3, stride=1, dilate=False):
...
def ResNet18(num_classes, pool_type, max_num_pools, noise_std, kap_kernelsize=3, sine_init=False):
    return ResNet(BasicBlock, [2, 2, 2, 2], planes=[64, 144, 256, 529], num_classes=num_classes,
    pool_type=pool_type, max_num_pools=max_num_pools, noise_std=noise_std, kap_kernelsize=kap_kernelsize, sine_init=sine_init)


def ResNet18WideX4(num_classes, pool_type, max_num_pools, noise_std, kap_kernelsize=3, sine_init=False):
    return ResNet(BasicBlock, [2, 2, 2, 2], planes=[64*4, 144*4, 256*4, 529*4], num_classes=num_classes,
    pool_type=pool_type, max_num_pools=max_num_pools, noise_std=noise_std, kap_kernelsize=kap_kernelsize, sine_init=sine_init)


def ResNet18WideX9(num_classes, pool_type, max_num_pools, noise_std, kap_kernelsize=3, sine_init=False):
...
def ResNet50(num_classes, pool_type, max_num_pools, noise_std, kap_kernelsize=3):
    return ResNet(Bottleneck, [3, 4, 6, 3], planes=[64, 144, 256, 529], num_classes=num_classes,
    pool_type=pool_type, max_num_pools=max_num_pools, noise_std=noise_std, kap_kernelsize=kap_kernelsize)


def ResNet50WideX4(num_classes, pool_type, max_num_pools, noise_std, kap_kernelsize=3):
