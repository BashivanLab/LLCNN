...
class V1Map(TissueMap):
    ...
    def get_preferences(self, metric: Metric):
        if metric.name == angle_metric.name:
            print(self.orientation_preferences[self.unit_mask])
    ...
    def metric_difference_over_distance(
        ...
        curves = []
        preferences = self.get_preferences(metric)#.reshape(8, 8)
        #print("note", preferences.shape)
        #for _ in tqdm(range(num_samples), disable=not verbose):
            #indices = self.rng.choice(
             #   np.arange(len(self.positions)), size=(len(self.positions),), replace=False
            #)
       
        #sample_preferences = np.expand_dims(preferences,axis=-1)#preferences[indices, np.newaxis]
        sample_preferences = preferences[:, np.newaxis]
        #index = np.where(sample_preferences == 0)[0]
        #sample_preferences=sample_preferences[~index]
        #positions = self.positions[~index]
        
        
            #print(sample_preferences.shape)
        ...
        # pairwise distances
        dists = lower_tri(squareform(pdist(self.positions)))
        if shuffle:
            self.rng.shuffle(dists)
       
        dist_mask = np.nonzero(dists <= distance_cutoff)[0]

        #print(dists.shape, diffs.shape)
        # relationships
    ...
    def make_parameter_map(
        ...
        **kwargs,
        ...
        point_sizes = (
            final_psm  * axis_point_scale * selectivity #multiplier
        )
