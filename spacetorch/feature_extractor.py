...
class FeatureExtractor:
    ...
    def extract_features(
        ...
        for i, layer in enumerate(model_layer):
            hook = self.get_hook(layer, f"{i}", target_dict=layer_results)
            hooks.append(hook)
        ...
        self.layer_feats = [v for k, v in self.layer_feats.items()]
        
        
        print(np.array(self.layer_feats).shape)
        # corner case: for a single layer, just return features
        if len(self.layer_feats) == 1:
            self.layer_feats = self.layer_feats[0]
